...
fi_index = FollowingIndex(1, 2, 0.301241, 0.137025, 0.346914, 7.07211)


net = read_nnet(net_path);
obs_radius = 0.5
USE_IA_FLAG = true
# choose solver: NNDynTrackGurobi(),  NNDynTrack():CPLEX 
...
function following(rp::RP, ctrl; fps=10, tf=2, targets=nothing, safety_index=nothing, verbose=false)
    ...
    for i in 1:T-1
        ...
        xref = Xref[1]
        Xrefs[i+1] = xref
        timed_result = @timed get_control(ctrl, xref, x, rp.net, rp.obj_cost, dt, obstacles=[targets[i]], safety_index=safety_index, IA_bounds=USE_IA_FLAG,p=P_NORM)
...
function following_stat(num, fi; ctrl=nothing, verbose=false)
    ...
    while j < num
        ...
        isnothing(Xtrack) && (@assert infeas==true; j -= 1; infeas_cnt += infeas; continue) 
        ...
        phi0_vio_cnt += vio
        net_path == "unicycle-FC2-100-rk4-extra/epoch_1000.nnet" && (vio=false) 
...
if SIS
    ...
    Random.seed!(1)
    res = bboptimize(eval_following_index; SearchRange = [(1e-3, 0.5), (1e-3, 10.), (0.1,10.), (0.1, 10)], TraceMode=:verbose, MaxFuncEvals=200, TargetFitness=0.0, FitnessTolerance=1e-6);

end

if VISUALIZE
    fps = 10
    (net_path == "unicycle-FC4-100-rk4-extra/epoch_1000.nnet") || (net_path == "unicycle-FC3-200-rk4-extra/epoch_1000.nnet") ? tf = 1 : tf = 8
    pos = [1.5, 1.5]
    v = [0.3, 0.4]
    targets = generate_moving_target(fps=fps, tf=tf, v= v, pos=pos)
    ...
    nv_ctrl = NvController(err_bound,inputs_bounds=[1, π], warm_start=true, bin_precision=2,solver = BPO_SOLVER)

    @assert phi(fi_index, x0, targets[1]) < 0 phi(fi_index, x0, targets[1])

    Xtrack, Utrack, safe_sets, Xrefs, Xopts, infeas, tot_time, time_vec = following(rp, nv_ctrl, fps=fps, tf=tf, targets=targets, safety_index=fi_index, verbose=false);
...
if STATS
    ...
    n=10
    success_rate, phi0_vio_rate, infeas_rate, mean_time, mean_error, std_error = following_stat(n, fi_index, ctrl=nv_ctrl, verbose=true)
