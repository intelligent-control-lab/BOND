...
net_path = "unicycle-FC3-100-rk4/epoch_1000.nnet"
ci_index = CollisionIndex(0.790279, 0.330962, 0.150041, 0.637638, 4.55352)
...
function get_Xref_new(x0, xg, T, dt)
    tf = T*dt
    ...
    vy = v * sin(a)
    w = da / tf
    Xref = [[x0[1]+vx*k*dt, x0[2]+vy*k*dt, v, x0[4] + w*k*dt] for k = 1:T]
    Xref[end][3] = 0
    return Xref
end
...
function tracking(rp::RP, ctrl; fps=10, tf=2, obstacles=nothing, safety_index=nothing, verbose=false)
    ...
    for i in 1:T-1
        ...
        xref = Xref[1]
        Xrefs[i+1] = xref
        timed_result = @timed get_control(ctrl, xref, x, rp.net, rp.obj_cost, dt, obstacles=obstacles, safety_index=safety_index, IA_bounds=USE_IA_FLAG, BPO_degree=BPO_DEGREE,p=P_NORM,worstcase=WORSTCASE)
        ...
        ((timed_result.time < 0.2) && USE_IA_FLAG && BPO_SOLVER == NNDynTrack() && P_NORM==2) && (val_time += timed_result.time)
        ((timed_result.time < 0.2) && USE_IA_FLAG && BPO_SOLVER == NNDynTrack() && P_NORM==2) &&  (valid_num += 1)
        ...
        X[i+1] = x
        @show i, x, xref, x + last_z * dt
        U[i] = u
        ((timed_result.time < 0.2) && USE_IA_FLAG && BPO_SOLVER == NNDynTrack() && P_NORM==2) && (@show i, val_time, valid_num)
...
function exists_valid_control(safety_index, ctrl::NvControllerBPO, x, obs, net, dt; IA_bounds=true, bpo_degree=1)
    # try to make the safeset as the worstcase one, to opt safety index in a blackbox way
    worstcase_safe_set = phi_safe_set(safety_index, x, obs, dt)
    obj_cost = [0,0,0,0]
    ...
    output = intersection(output, worstcase_safe_set)
    isempty(output) && return false
    problem = TrackingProblem(net, input, output, dot_x_ref, obj_cost* (dt^2))
    result, start_values = solve_BPO_worstcase_new(ctrl.solver, problem, ctrl.start_values, u_ref=nothing, xu_init=nothing, IA_bounds=IA_bounds, BPO_degree=bpo_degree)
...
function eval_collision_index(coes; whole_space=true)
    ...
    index = CollisionIndex(margin, gamma, zeta, phi_power, dot_phi_coe)
    valid = 0
    net = read_nnet(net_path);
    dt = 0.1
    tot_cnt = 0
    ...
    for i in ProgressBar(1:length(col_samples_flat))
        ...
        evc = exists_valid_control(index, ctrl, x, obs, net, dt; IA_bounds=USE_IA_FLAG, bpo_degree=BPO_DEGREE)
        valid += evc
    end
...
function collision_stat(num, ci; ctrl=nothing, verbose=false)
    ...
    return success*1.0/total_num, phi0_vio_cnt*1.0/total_num, infeas_cnt*1.0/total_num, tot_time / total_steps, mean(all_error), std(all_error)#, mean(all_pos), std(all_pos), mean(all_vel), std(all_vel), mean(all_angle), std(all_angle)
end
...
function myvisualize(X; Xref=nothing, Xmpcs=nothing, xlims = nothing, ylims = nothing, obstacles=nothing, targets=nothing, safe_sets=nothing, save_name=nothing, fps=10, save_frame=nothing, traj_label=nothing, time_vec=nothing)
    ...
    for i = 1:step-1
        ...
        l = 0.8
        vx, vy, w1x, w1y, w2x, w2y = get_unicycle_endpoints(x, l)
        ...
        if !isnothing(safe_sets)
            if isa(safe_sets[i], HalfSpace)
                plot!(HalfSpace(safe_sets[i].a[1:2], safe_sets[i].b * dt + safe_sets[i].a[1]*xs[i]+safe_sets[i].a[2]*ys[i]), label="Safety Constraint", color=2)
            else
...
function myvisualize_new(X; Xref=nothing, Xopt=nothing, Xmpcs=nothing, xlims = nothing, ylims = nothing, obstacles=nothing, targets=nothing, safe_sets=nothing, save_name=nothing, fps=10, save_frame=nothing, traj_label=nothing, time_vec=nothing)
    step = length(X)
    @assert step==3
    if !isnothing(Xref)
        xrefs = [Xref[i][1] for i in 1:length(Xref)]
    ...
    if !isnothing(Xopt)
        xopts = [Xopt[i][1] for i in 1:length(Xopt)]
        yopts = [Xopt[i][2] for i in 1:length(Xopt)]
    end
    xs = [X[i][1] for i in 1:length(X)]
    ys = [X[i][2] for i in 1:length(X)]
    dt = 1 / fps
    ...
    for i = 1:step-1
        ...
        if i == 2
            x = X[i-1]
            vx, vy, w1x, w1y, w2x, w2y = get_unicycle_endpoints(x, 0.24)
            plot!(p, w1x, w1y, linewidth=43, color=:gray, label="")
            plot!(p, w2x, w2y, linewidth=43, color=:gray, label="")
            plot!(p, vx, vy, linewidth=34, xlims = xlims, ylims = ylims, color=:navajowhite3, label="", aspect_ratio=:equal, dpi=dpi, legend=:bottomright)
        ...
        if !isnothing(Xref)
            @show xrefs[1:i], yrefs[1:i]
            plot!(xrefs[1:i], yrefs[1:i], label="Reference Motion", color=:green, linewidth=3)
        end
        if !isnothing(Xopt)
            @show xopts[1:i], xopts[1:i]
            plot!(xopts[1:i], yopts[1:i], label="Optimized Motion", color=:blue, linewidth=3)
        end
        if !isnothing(Xmpcs)
            plot!([Xmpcs[i][j][1] for j in 1:length(Xmpcs[i])], [Xmpcs[i][j][2] for j in 1:length(Xmpcs[i])], label="Xmpc")
            scatter!([Xmpcs[i][1][1]], [Xmpcs[i][1][2]], label="current mpc")
        end
        plot!(xs[1:i], ys[1:i], label=isnothing(traj_label) ? "Executed Motion" : traj_label, color=:red,linewidth=3)
        if !isnothing(safe_sets)
            if isa(safe_sets[i], HalfSpace)
                if i > 1
                    plot!(HalfSpace(safe_sets[i].a[1:2], safe_sets[i].b * dt + safe_sets[i].a[1]*xs[i-1]+safe_sets[i].a[2]*ys[i-1]), label="Safety Constraint", color=2)
                end
            else
                safe_set = reduce(intersection, [HalfSpace(con.a[1:2], con.b* dt + con.a[1]*xs[i]+con.a[2]*ys[i]) for con in safe_sets[i].constraints])
                plot!(safe_set, label="Safety Constraint")
...
if SIS
    ...
    addprocs(2)
    Random.seed!(0)
    x0 = [0.1, 2.5, 0.1,  2.0, 1.0]
    res = bboptimize(eval_collision_index, x0; SearchRange = [(1e-3, 1), (1e-3, 5),(1e-3, 1), (0.1, 5.0), (0.1, 5.0)], TraceMode=:verbose, MaxFuncEvals=100, TargetFitness=0.0, FitnessTolerance=1e-6);
...
if VISUALIZE
    ...
    nv_ctrl_bpo = NvControllerBPO(err_bound,inputs_bounds=[2, π], warm_start=true, bin_precision=2, solver=BPO_SOLVER)

    
    @assert phi(ci_index, x0, obstacles[1]) < 0
    
    ((BPO_DEGREE == 2) && (BPO_SOLVER == NNDynTrackGurobi()) && (net_path != "unicycle-FC2-100-rk4-extra/epoch_1000.nnet")) ? tf=1 : tf=50

    Xtrack, Utrack, safe_sets, Xrefs, Xopts, infeas, time, time_vec = tracking(rp, nv_ctrl_bpo, fps=10, tf=tf, obstacles=obstacles, safety_index=ci_index, verbose=false);

    local min_dist = 10000
    local max_dist = 0
    for obs in obstacles
        for x in Xtrack
            if  norm(x[1:2]-obs.center) < min_dist
                min_dist = norm(x[1:2]-obs.center)
            end 
            if  norm(x[1:2]-obs.center) > max_dist
                max_dist = norm(x[1:2]-obs.center)
            end 
        end
    end
    @show min_dist, max_dist
    ...
    myvisualize(Xtrack, obstacles=obstacles, xlims=[-1,6], ylims=[-1,6], fps=10,safe_sets=safe_sets)
...
if STATS
    err_bound = [1, 1, 0.1, 0.1] * 10^9 
    nv_ctrl = NvControllerBPO(err_bound,inputs_bounds=[2, π], warm_start=true, bin_precision=2,solver = BPO_SOLVER)
